  ...
  return (
    <div className="context-probe">
      ...
      <div className="context-probe__main-card">
        ...
        {/* 反馈层 - 浮动显示 */}
        {submitted && (
          <div className={`context-probe__feedback-overlay ${isCorrect ? 'is-pass' : 'is-fail'}`}>
            <div className="feedback-icon">
              {isCorrect ? <CheckCircle2 size={32} /> : <XCircle size={32} />}
            </div>
            <div className="feedback-content">
              <h4>{isCorrect ? '✅ 猜对 (Pass) → 跳级' : '❌ 猜错 (Fail) → 下沉'}</h4>
              <p>
                {isCorrect 
                  ? '判定为"疑似熟词"，将直接跳去 Phase 3 验真' 
                  : '标记为 ⚪ Pending（待处理），推入 Phase 2 训练'}
              </p>
            </div>
          </div>
        )}
